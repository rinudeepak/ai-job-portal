...
class DashboardController extends BaseController
{
    ...
    public function index()
    {
        ...
        $funnel = [
            ...
            'screening' => $applicationModel->where('status', 'pending')->countAllResults(),
            'ai_interview_scheduled' => $applicationModel->where('status', 'ai_interview_scheduled')->countAllResults(),
            ...
            'hr_interview_scheduled' => $applicationModel->where('status', 'hr_interview_scheduled')->countAllResults(),
            'hr_interview_completed' => $applicationModel->where('status', 'hr_interview_completed')->countAllResults(),
            'selected' => $applicationModel->where('status', 'selected')->countAllResults(),
    ...
    public function leaderboard()
    {
        ...
        $builder = $applicationModel
            ...
            ->where('applications.overall_rating IS NOT NULL')
            ->where('applications.overall_rating >', 0);
        ...
        // Apply skill filter (search in required_skills JSON)
        if ($skill) {
            $builder->like('jobs.required_skills', $skill);
        }
        ...
        $candidates = $builder->paginate(50);
    ...
    private function getLeaderboardExportData()
    {
        ...
        $data = [
            ['Rank', 'Name', 'Email', 'Job', 'Technical Score', 'Communication Score', 'Overall Rating', 'Status']
        ];
    ...
    private function getDetailedExportData()
    {
        ...
        $applications = $applicationModel
            ->select('applications.*, users.name, users.email, users.phone, jobs.title as job_title,
            interview_sessions.overall_rating')
            ->join('users', 'users.id = applications.candidate_id', 'left')
            ->join('jobs', 'jobs.id = applications.job_id', 'left')
            ->join('interview_sessions', 'interview_sessions.application_id = applications.id', 'left')
        ...
        $data = [
            [
                'ID', 'Name', 'Email', 'Phone', 'Job', 'Status', 
                'Technical Score', 'Communication Score', 'Overall Rating',
    ...
    private function getStageCount($stage)
    {
        ...
        $statusMap = [
            ...
            'Screening' => 'pending',
            'AI Interview Scheduled' => 'ai_interview_scheduled',
            'AI Interview Completed' => 'ai_interview_completed',
            'Shortlisted' => 'shortlisted',
            'HR Interview Scheduled' => 'hr_interview_scheduled',
            'HR Interview Completed' => 'hr_interview_completed',
            'Selected' => 'selected',
            'Rejected' => 'rejected'
        ];
